...
class TextTokenizeRequest(BaseModel):
    ...
    max_tokens: int = Field(default=5000, ge=1, le=10_000)


class TokenizedTokenRecord(TokenRecord):
...
class TextTokenizeResponse(BaseModel):
    ...
    projectable_token_count: int = Field(ge=0)
    add_special_tokens: bool
    truncated: bool
    tokens: list[TokenizedTokenRecord]
...
class VectorExpressionRequest(BaseModel):
...
class ProjectionRequest(BaseModel):
    ...
    geometry_mode: Literal["sphere", "euclidean"] = "sphere"
    arithmetic_expressions: list[VectorExpressionRequest] = Field(default_factory=list, max_length=12)
...
class ProjectionPoint(BaseModel):
    ...
    kind: Literal["token", "resultant"] = "token"
    alias: str | None = None
    label: str | None = None
    expression: str | None = None
    referenced_aliases: list[str] = Field(default_factory=list)
