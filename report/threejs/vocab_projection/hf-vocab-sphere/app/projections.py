...
PROJECTION_METHODS: dict[str, dict[str, Any]] = {
    "auto": {
        ...
        "family": "adaptive",
        "complexity": "Chooses by token count and anchor availability",
    ...
    "spherical_pca": {
        ...
        "caveat": "PCA optimizes variance, not angular-distance fidelity; radial sphere normalization adds distortion.",
        "max_points": 5000,
        "stochastic": False,
    },
    "tangent_pca": {
    ...
    "angular_mds": {
        ...
        "caveat": "Classical MDS is Euclidean; the final radial projection onto S² introduces additional stress.",
        "max_points": 800,
        "stochastic": False,
    },
    "spherical_stress": {
        ...
        "caveat": "Iterative and non-convex; it preserves neither every angle nor a unique global orientation.",
        "max_points": 500,
        "stochastic": True,
    },
    "isomap": {
...
def _orient_sphere(coords: np.ndarray, anchor_index: int | None) -> np.ndarray:
    ...
    dots = np.clip(coords @ coords[anchor_index], -1.0, 1.0)
...
def _choose_auto_method(n: int, anchor_index: int | None) -> str:
    if n <= 140:
...
def projection_metrics(
    ...
    *,
    ...
    low_cos = np.einsum("ij,ij->i", sphere_coords[rows], sphere_coords[cols])
    high_angles = np.arccos(np.clip(high_cos, -1.0, 1.0))
    low_angles = np.arccos(np.clip(low_cos, -1.0, 1.0))
    error_deg = np.degrees(np.abs(low_angles - high_angles))
    denom = float(np.sum(high_angles * high_angles))
    stress = math.sqrt(float(np.sum((low_angles - high_angles) ** 2)) / max(denom, _EPS))
    ...
    low_probe_similarity = sphere_coords[probe_rows] @ sphere_coords.T
    recalls: list[float] = []
    for probe_index, row in enumerate(probe_rows):
        high_similarity = high_probe_similarity[probe_index]
        low_similarity = low_probe_similarity[probe_index]
    ...
    if anchor_index is not None and 0 <= anchor_index < n:
        ...
        low_anchor_angles = np.arccos(np.clip(sphere_coords @ sphere_coords[anchor_index], -1.0, 1.0))[mask]
        anchor_rho = _safe_spearman(high_anchor_angles, low_anchor_angles)

    return {
        "pair_sample_count": int(len(high_angles)),
        "angular_spearman_rho": _safe_spearman(high_angles, low_angles),
...
def project_vectors(
    ...
    *,
    ...
    actual = _choose_auto_method(n, anchor_index) if requested == "auto" else requested
    ...
    sphere, sphere_warnings = _sphere_normalize(raw, seed)
    warnings.extend(sphere_warnings)
    if align_anchor:
        sphere = _orient_sphere(sphere, anchor_index)
    ...
    metrics["radial_normalization"] = True
    details["sphere_mapping"] = "radial normalization of the 3-D embedding"
    details["anchor_aligned_to_north"] = bool(align_anchor and anchor_index is not None)

    return ProjectionOutput(
        coordinates=sphere.astype(np.float32),
