#!/bin/bash
# Exit immediately if any command fails
set -e

# ==========================================
# CONFIGURATION
# ==========================================
# Change to "Qwen/Qwen3.5-0.6B-Instruct" to test Instruct capabilities
MODEL_ID="Qwen/Qwen3.5-0.6B"

echo "=========================================="
echo "🚀 STARTING QWEN-SIGLIP2 PIPELINE"
echo "Target Model: $MODEL_ID"
echo "=========================================="

# 1. Run the training script (using map_only mode for faster global token training)
echo -e "\n\n>>> 1/4: RUNNING TRAINING <<<"
python qwen_siglip2_finetune.py --train_mode map_only --model_id "$MODEL_ID"

# 2. Setup a test image
TEST_IMAGE="test_image.jpg"
if [ ! -f "$TEST_IMAGE" ]; then
    echo -e "\n📥 Downloading a sample image for testing..."
    # Downloads a standard picture of a dog
    wget -qO $TEST_IMAGE "https://images.unsplash.com/photo-1543466835-00a7907e9de1?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80"
fi

# 3. Test Mode 1: Zero-Shot Category
echo -e "\n\n>>> 2/4: TESTING CATEGORY MODE <<<"
python qwen_siglip2_inference.py $TEST_IMAGE \
    --mode category \
    --concepts dog cat bird car \
    --model_id "$MODEL_ID"

# 4. Test Mode 2: Dense Captioning
echo -e "\n\n>>> 3/4: TESTING CAPTION MODE (DENSE PATCHES) <<<"
python qwen_siglip2_inference.py $TEST_IMAGE \
    --mode caption \
    --query "Describe the main subject of this image in detail." \
    --model_id "$MODEL_ID"

# 5. Test Mode 3: MAP Query
echo -e "\n\n>>> 4/4: TESTING MAP QUERY MODE (GLOBAL TOKEN) <<<"
python qwen_siglip2_inference.py $TEST_IMAGE \
    --mode map_query \
    --query "Describe the main subject of this image in detail." \
    --model_id "$MODEL_ID"

echo -e "\n=========================================="
echo "✅ PIPELINE COMPLETE!"
echo "=========================================="
