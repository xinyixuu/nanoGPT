...
SWEEP_MAX_ITERS="${SWEEP_MAX_ITERS:-1000}"
...
for num_digits in ${DIGIT_COUNTS}; do
  for num_letters in ${LETTER_COUNTS}; do
    for mode in unconstrained sqrt_dim unit; do
      ...
      name="digits-${num_digits}_letters-${num_letters}_${mode}"
      echo "=== ${name} ==="
...
Example result:
  http://localhost:8000/report/threejs/digits-3d/index.html?data=runs/digits-10_letters-4_sqrt_dim.json
EOF
